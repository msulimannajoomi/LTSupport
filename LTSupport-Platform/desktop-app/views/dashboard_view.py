...
class DashboardView(ctk.CTkFrame):
    def __init__(self, parent, app):
        ...
        host_card = self._action_card(cards, "🖥️ Host This Computer",
                                       ...
                                       "Device ID with.",
                                       theme.SUCCESS, app.show_host, disabled=blocked, hover_color="#22c55e")
    ...
    def _render_plan_banner(self):
        ...
        if api.blocked:
            ...
            text = f"TRIAL PLAN — 2-minute session limit · {api.session_count} session(s) so far."
        elif api.account_type == "prepaid":
            text = f"PREPAID PLAN — {api.balance_minutes:.1f} minute(s) remaining."
        else:
